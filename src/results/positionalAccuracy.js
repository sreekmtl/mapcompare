...
import { lineCompleteness } from './completeness';
...
export function linePositionalAccuracy(vectorLayer1, vectorLayer2){
    ...
    let buffer1= buffer(bufferedLine, 0.002,'kilometers');
    ...
    for (let i=0; i<dissolvedFeatures.length;i++){
        ...
        for (let j=0; j<lineFeatures.length;j++){
            ...
            let line_overlap= booleanIntersects(lin_ls,dis_poly);
            if (line_overlap===true){
                
                let intersects= lineSplit(lin_ls,dis_poly) //This return featurecollection of points which have intersected
                if (intersects.features.length>=1){
                    overlappedFeatures.push(...intersects.features) //This contains individual intersected point features
                }
            }
            if (booleanWithin(lin_ls,dis_poly)){
